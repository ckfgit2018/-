...
Page({
    data: {
        ...
        height: 0,
        selectType:'',
        switchBar: [{ text: '地区', isActive: false,type:'address'}, { text: '分类', isActive: false,type:'classify' }],
        animationData: {},
    },
    onLoad: function(t) {
        app.pageOnLoad(this);
    ...
    onShow: function() {
        ...
        let list=cat_list[0].list;
        let list1=list.slice(0,8);
        let list2=list.slice(8,16);
        let list3=list.slice(16,list.length);
        let classify=cat_list.filter((item)=>{ return item.id!=7});
        this.setData({
            list1:list1,
            list2:list2,
            list3:list3,
    ...
    goodsItem: function(t) {
        let switchBar=this.data.switchBar;
        switchBar.map((item)=>{item.isActive=false})
        var a = t.currentTarget.dataset.index,
    ...
    goodsAll: function(t) {
        ...
        var c = t.currentTarget.offsetLeft,
            l = a.data.scrollLeft;
        l = c - 80, a.setData({
            scrollLeft: l
    ...
    switch (e) {
        ...
        let switchBar=this.data.switchBar;
        if(!switchBar[index].isActive){
            switchBar.map((item)=>{item.isActive=false})
            switchBar[index].isActive=true
        }else{
            switchBar.map((item)=>{item.isActive=false})
            switchBar[index].isActive=false 
        }
        this.setData({
            switchBar:switchBar,
            selectType:switchBar[index].type
        })
    }
});